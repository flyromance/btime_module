import { mix, createEvent, throttle } from '@/component/util'
import fetchData from '@/component/ajax'
...
Scroll.prototype = {
    ...
    bindEvent: function () {
        ...
        // 滚动加载事件
        $(window).on(that.scrollLoadName, throttle(function (e) {
            if (!that.loading) {
                that.handleScroll(e);
            }
        }, that.conf.thresholdTime));
        setTimeout(function () {
            $(window).trigger(that.scrollLoadName)
        }, 0)
    },
    setLoadingState: function (flag) {
        this.loading = flag;
...
InfoFlow.prototype = {
    ...
    init: function (conf) {
        ...
        // 滚动加载
        that.scroll.module = that.conf.hasScrollLoad ? new Scroll(that) : null
    ...
    handleIndex: function () {
        ...
        if (that.conf.hasRemindLoad) {
            ...
            // 处理首屏已有数据（不用请求的情况)
            that.handleData({
                page: ++that.scroll.page, // 有首屏数据，推荐次数加1
                strategy: 'scroll',
                resData: that.conf.firstPageData,
                $container: that.$container,
                module: that
    ...
    handleData: function (obj) {
        ...
        // 是否要去重
        if (that.conf.isNoRepeat && obj.hasData) {
            that.trigger('removeRepeat', obj);
        }
    ...
    handleNoData: function (obj) {
        ...
        // 属于初次没有数据的情况，特殊处理
        if (obj.page == 1) {
            that.$container.html(that.conf.initNoDataUi);
        } else {
            that.afterHandleData(obj);
        }

        that.trigger('handleNoData', obj);
    },
    ...
    bindEvent: function () {
        ...
        that.bind('preHandleData', conf.preHandleData || function (obj) {
            obj.data = obj.resData;
        });
        ...
        // 处理没有数据时，ui提示
        if (conf.handleNoData) {
            that.bind('handleNoData', conf.handleNoData);
        }

        // 处理重复数据
        if (conf.removeRepeat) {
            that.bind('removeRepeat', conf.removeRepeat);
        }
